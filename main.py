...
# Note: Ensure you are using a multimodal model like gemini-1.5-flash or gemini-1.5-pro
...
app = FastAPI(
    title="Class 8-12 AI Tutor",
    description="Multimodal backend for ICSE, CBSE, & SSLC tutoring",
    version="1.1.0",
)
...
def get_intro(payload: dict):
    ...
    board_text = f"{board} Class" if board in ALLOWED_BOARDS else "Class"

    intro = (
        f"Hello! I'm here to help you with your {board_text} "
        f"{class_level} {subject} questions.\n\n"
        "You can type your question or upload a photo/PDF of your textbook problem!"
    )
    return {"intro": intro}
...
def ask_question(payload: dict):
    ...
    # 2. Build Content Parts (Multimodal)
    # Part 1: The Instruction Prompt
...
* if this bard is selected, give answers with reference to the latest SSLC KARNATAKA BOARD syllabus
    ...
    content_parts = [prompt_instructions]
    
    if question_text:
        content_parts.append(f"Student Question: {question_text}")

    # Part 2: Process Files (Images/PDFs)
    for file_obj in attachments:
        try:
            # Clean base64 string (remove data:image/... prefix if present)
            raw_data = file_obj["data"]
            if "," in raw_data:
                raw_data = raw_data.split(",")[1]
            
            # Determine MIME type (default to image/jpeg if missing)
            m_type = file_obj.get("mime_type") or file_obj.get("type") or "image/jpeg"
    ...
    try:
        ...
        if not answer:
            answer = "I couldn't read the file or the question. Please try uploading a clearer image."
